...
import {getAccessToken, decryptAt} from '../../Lib/Utils'
...
export function * loginDoLogin (api, action) {
  ...
  let userRole = path(['data', 'user', 'userRole'], response) || ''
  // let userMerchantCodeArr = path(['data', 'user', 'merchant'], response) || [{}]
  let userMerchantCode = path(['data', 'user', 'merchantCode'], response) || '' // 000000070070070
  let userMerchantId = path(['data', 'user', 'merchantId'], response) || '' || '8afb8b146d7212fa016d721bb5970005'
  ...
  if (responseCode === 'MBDD00') {
    return yield put(LoginActions.loginDoLoginSuccess({sessionToken, publicToken, responseCode, responseMessage, responseDescription, userFullName, userRole, userMerchantCode, userMerchantId}))
  } else {
    return yield put(LoginActions.loginDoLoginFailed({isRequesting: false, responseCode, responseMessage, responseDescription, userFullName: '', userRole: '', userMerchantCode: '', userMerchantId: ''}))
  }
}
export function * loginCheckStatus (api, action) {
  ...
  let responseCode = path(['data', 'responseCode'], response)
  let responseMessage = path(['data', 'responseMessage'], response)
  if (response.ok && responseCode === 'MBDD04') {
    console.log('LOGOUT')
    return yield call(doLogout)
  }
