...
export const removeSuccess = (state, action) => {
  // const { payload } = action
  return loginDoLogoutSuccess(state, action)
...
export const singleSuccess = (state, action) => {
  ...
  setSession({[AppConfig.loginFlag]: isLoggedIn})
...
export const loginDoLoginFailed = (state, { data }) => {
  console.log('loginDoLoginFailed')
...
export const loginDoLogoutSuccess = (state, { data }) => {
  ...
  setSession({'userRole': '', [AppConfig.loginFlag]: false, [AppConfig.sessionToken]: ''})
  data.isRequesting = false
  data.isLoggedIn = false
